...
import AdminLayout from "../../components/admin/AdminLayout"; // Update if path differs
              ...
              {(grouped[status] || []).map((c) => (
                <div key={c._id} className="border p-4 rounded shadow bg-white">
                  ...
                  <p className="text-sm text-gray-600">
                    Student: {c.studentName || "N/A"} ({c.student}), Room: {c.roomNumber || "N/A"}
                  </p>
                  {c.resolvedAt && (
                    <p className="text-xs text-green-600 mt-1">Resolved At: {new Date(c.resolvedAt).toLocaleString()}</p>
                  )}
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded shadow-lg max-w-md w-full">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => setSelectedComplaint(null)} className="px-4 py-2 bg-gray-300 rounded">
                Cancel
              </button>
              <button
